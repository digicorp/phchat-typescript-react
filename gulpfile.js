...
gulp.task(
  BUILD,
  gulp.series(CLEAN_BUILD, TSLINT, COMPILE_TYPESCRIPT, COPY_STATIC_FILES)
);

// Generates a documentation based on the code comments in the *.ts files.
gulp.task(
  GENERATE_DOC,
  gulp.series(CLEAN_DOC, function () {
    return gulp.src(TS_SRC_GLOB).pipe(
      typedoc({
        out: "docs",
        target: "es5",
...
gulp.task(REMAP_COVERAGE, function () {
  return gulp
    ...
    .on("end", function () {
      console.log(
        "--> For a more detailed report, check the ./coverage directory <--"
      );
    });
});
...
gulp.task(
  TEST,
  gulp.series(BUILD, CLEAN_COVERAGE, PRETEST, RUN_TESTS, REMAP_COVERAGE)
);
